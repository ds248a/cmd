...
func main() {
	err := Run(context.Background(), []string{"ls"})
	if err != nil {
		fmt.Printf("err %v\n", err)
	}
}
...
//
func Run(ctx context.Context, param []string) error {
	...
	// запуск комманды
	cmdExec := exec.CommandContext(cmdCtx, param[0], param[1:]...)
	...
	// timer
	timer := time.NewTimer(time.Duration(int(time.Millisecond) * defaultDelayToKill))
